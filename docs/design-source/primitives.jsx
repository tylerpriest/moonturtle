// Reusable visual primitives (glyphs, moon, turtle, ornaments)
const { useState, useEffect, useRef } = React;

// ── Turtle wordmark ──
function TurtleMark({ size = 26, color = "currentColor" }) {
  return (
...
// ── Moon phase SVG (illumination % + waxing/waning) ──
function MoonGlyph({ size = 40, illumPct = 74.6, waxing = false }) {
  const r = size / 2 - 1;
  ...
  const w = phaseFrac <= 0.5; // waxing portion drawing
...
// ── Botanical sprig ──
function Sprig({ size = 70, flip = false, opacity = 0.35 }) {
  return (
  ...
  return (
    <div className="status-bar">
      ...
      <div className="status-icons">
        ...
        <svg width="14" height="10" viewBox="0 0 14 10"><path d="M7 9.5c-3 0-5.5-1.5-7-4 1.5-2.5 4-4 7-4s5.5 1.5 7 4c-1.5 2.5-4 4-7 4z" fill="none" stroke="currentColor" strokeWidth="0.8"/><circle cx="7" cy="5.5" r="2" fill="currentColor"/></svg>
        <svg width="22" height="10" viewBox="0 0 22 10"><rect x="0.5" y="0.5" width="18" height="9" rx="2" fill="none" stroke="currentColor"/><rect x="2" y="2" width="14" height="6" rx="1" fill="currentColor"/><rect x="19" y="3.5" width="2" height="3" fill="currentColor"/></svg>
      </div>
    </div>
  );
}
...
// ── Phone wrapper ──
function Phone({ children, withTabBar, activeTab, onTab, fixed }) {
  return (
    <div className={`phone${fixed?' phone-fixed':''}`}>
      <StatusBar/>
      <div className="screen-body">{children}</div>
      {withTabBar && <TabBar active={activeTab} onTab={onTab}/>}
    </div>
  );
}
...
function TabBar({ active, onTab }) {
  const tabs = [
    { id: 'today',    label: 'Today',    icon: <SunGlyph size={20}/> },
    { id: 'sky',      label: 'Sky',      icon: <MoonGlyph size={18} illumPct={74.6}/> },
    { id: 'natal',    label: 'Natal',    icon: <svg width="20" height="20" viewBox="0 0 20 20"><circle cx="10" cy="10" r="8" fill="none" stroke="currentColor" strokeWidth="0.8"/><circle cx="10" cy="10" r="4" fill="none" stroke="currentColor" strokeWidth="0.8"/><line x1="2" y1="10" x2="18" y2="10" stroke="currentColor" strokeWidth="0.6"/><line x1="10" y1="2" x2="10" y2="18" stroke="currentColor" strokeWidth="0.6"/></svg> },
    { id: 'journal',  label: 'Journal',  icon: <svg width="18" height="20" viewBox="0 0 18 20"><rect x="1" y="1" width="16" height="18" fill="none" stroke="currentColor" strokeWidth="0.8"/><line x1="4" y1="6"  x2="14" y2="6" stroke="currentColor" strokeWidth="0.6"/><line x1="4" y1="10" x2="14" y2="10" stroke="currentColor" strokeWidth="0.6"/><line x1="4" y1="14" x2="11" y2="14" stroke="currentColor" strokeWidth="0.6"/></svg> },
    { id: 'method',   label: 'Method',   icon: <svg width="20" height="20" viewBox="0 0 20 20"><circle cx="10" cy="10" r="8" fill="none" stroke="currentColor" strokeWidth="0.8"/><text x="10" y="14" textAnchor="middle" fontSize="11" fontFamily="serif" fontStyle="italic" fill="currentColor">i</text></svg> },
  ];
  return (
    <div className="tab-bar">
      {tabs.map(t => (
        <div key={t.id} className={`tab${active===t.id?' active':''}`} onClick={()=>onTab&&onTab(t.id)}>
          {t.icon}
          <span>{t.label}</span>
        </div>
      ))}
...
Object.assign(window, { TurtleMark, Wordmark, MoonGlyph, Sprig, SunGlyph, OrnamentDiv, StatusBar, Phone, TabBar });
