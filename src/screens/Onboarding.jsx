...
function FormField({ label, value, hint }) {
  return (
    <div style={{marginBottom:22}}>
      ...
      {hint && (
        <div className="meta" style={{marginTop:6, fontFamily:'var(--serif)', fontStyle:'italic', fontSize:13, color:'var(--ink-mute)'}}>{hint}</div>
      )}
    </div>
  );
}
...
export function BirthSetup({ onNext }) {
  return (
    <div className="onboard-shell">
      <div style={{padding:'56px 30px 40px', position:'relative', flex:1}}>
...
export function Permission({ onDone }) {
  return (
    <div className="onboard-shell">
      <div style={{padding:'56px 30px 40px', flex:1, display:'flex', flexDirection:'column'}}>
        <Wordmark/>
        <div style={{height:50}}/>
        ...
        <button className="btn" onClick={onDone}>Allow &amp; Continue</button>
        <div style={{height:10}}/>
