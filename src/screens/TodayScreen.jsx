  ...
  return (
    <div style={{textAlign: right ? 'right' : 'left'}}>
      ...
      <div style={{display:'flex', flexDirection:'column', gap:3}}>
        {words.map(w => (
          <div key={w} style={{fontFamily:'var(--serif)', fontSize:13, color:'var(--ink-soft)', fontStyle:'italic', lineHeight:1.4}}>{w}</div>
        ))}
      </div>
    </div>
  );
...
function ActivationCard({ a, index }) {
  return (
    <div className="card" style={{padding:'18px 20px'}}>
      ...
      {a.insight && (
        <div style={{fontFamily:'var(--serif)', fontStyle:'italic', fontSize:15, color:'var(--ink)', borderLeft:'2px solid var(--terracotta)', paddingLeft:12}}>
          {a.insight}
        </div>
      )}
    </div>
...
export function TodayScreen({ onTab }) {
  return (
    <div style={{padding:'24px 26px 36px', position:'relative'}}>
      ...
      <div className="card" style={{padding:'22px'}}>
        <div style={{display:'grid', gridTemplateColumns:'1fr auto 1fr', gap:14, alignItems:'center'}}>
          <AxisCol
            label="Natal Moon"
            sign={d.lunarAxis.natal.sign}
            sub="12th house"
            words={d.lunarAxis.natal.words}
          />
          ...
          <AxisCol
            label="Sky Moon"
            sign={d.lunarAxis.current.sign}
            sub="now"
            words={d.lunarAxis.current.words}
            right
          />
        </div>
        <div style={{height:18}}/>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr', gap:14}}>
          ...
          <ul style={{listStyle:'none', display:'flex', flexDirection:'column', gap:10}}>
            {d.notice.map((n,i) => (
              <li key={i} style={{display:'flex', gap:10, alignItems:'flex-start'}}>
                <span style={{flexShrink:0, marginTop:7, width:6, height:6, borderRadius:'50%', background:'var(--moss)', display:'block'}}/>
        ...
        <div className="card" style={{borderTop:'2px solid var(--plum)'}}>
          ...
          <ul style={{listStyle:'none', display:'flex', flexDirection:'column', gap:10}}>
            {d.avoid.map((n,i) => (
              <li key={i} style={{display:'flex', gap:10, alignItems:'flex-start'}}>
                <span style={{flexShrink:0, marginTop:8, width:8, height:1, background:'var(--plum)', display:'block'}}/>
                <span className="body-prose" style={{fontSize:15}}>{n}</span>
              </li>
            ))}
          </ul>
